...
- **An attribute with a `default` is now Optional *and* Computed.** Terraform rejects a provider-supplied value on an attribute that is not computed -- `planned value cty.StringVal("small") for a non-computed attribute` -- so a default was unusable without it, and Optional + Computed is how the protocol spells "the practitioner may set this, and the provider fills it in otherwise". Required and write-only attributes are untouched, since neither can be computed. Unlike a plain Optional + Computed attribute, removing a defaulted attribute from the configuration plans a change back to the default rather than retaining the value in state: the effective configuration is what the resource reads at apply time, so it is also what gets planned, and a plan that kept a stale value while `ctx.config` reported the default would fail apply's refinement check.

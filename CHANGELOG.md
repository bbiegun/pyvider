...
- **An attribute with a `default` is now Optional *and* Computed.** Terraform rejects a provider-supplied value on an attribute that is not computed -- `planned value cty.StringVal("small") for a non-computed attribute` -- so a default was unusable without it, and Optional + Computed is how the protocol spells "the practitioner may set this, and the provider fills it in otherwise". Required and write-only attributes are untouched, since neither can be computed. The visible consequence is the usual Optional + Computed behaviour: once a value is in state, removing the attribute from the configuration leaves it in place rather than reverting to the default.

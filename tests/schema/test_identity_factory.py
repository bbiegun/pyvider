...
def test_s_identity_defaults_to_version_zero() -> None:
    """Identity versioning implicitly starts at 0, per the tfplugin6 proto.

    The default matters for adoption: an instance written before the resource
    declared identity carries IdentitySchemaVersion 0 in state, so a version-0
    identity schema matches and never triggers UpgradeResourceIdentity.
    """
    assert s_identity(attributes={"path": a_str(required=True)}).version == 0
...
def test_pvs_schema_version_validator_actually_raises() -> None:
    ...
    first code that can reach a bad value. Only negative versions are bad --
    Terraform's own built-in terraform_data resource declares Version: 0.
    """
    with pytest.raises(PvsSchemaDefinitionError, match="version"):
        PvsSchema(version=-1, block=PvsObjectType(attributes={}))


def test_pvs_schema_accepts_version_zero() -> None:
    assert PvsSchema(version=0, block=PvsObjectType(attributes={})).version == 0
